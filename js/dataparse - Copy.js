...
// ---------------------  PROGRAM START --------------------
for (let i = 0; i < 8; i++) {  // This is to test access to the array
  let y=keyPhrase[i];
  console.log(y.toString());  // output to eventually be routed to the json creator
...
// ------------- START OF PROCESSING TEXT FILE DATA
        ...
        if (line.startsWith("Device Name")) {    // This is the client-entered machine name
                dummyTxt = "Device Name"         // Model No. may be on the same line
...
// Serial Number: Remove the 2 zeroes padding the end, Sharp MFPs only use an 8 digit S/N         
        ...
        if (line.startsWith("Black & White Total Print Count")) {
          monoCount.textContent = line.slice(34, 42);
        ...
        if (line.startsWith("Color Total Print Count")) {
          colorCount.textContent = line.slice(24, 32);
             }            
      ...
      if ( color || tonerQuan || endRecordFlag)    // Ready Gate w/ Ready Flags
